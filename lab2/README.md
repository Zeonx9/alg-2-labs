...
Зависимость времени ответа для 10000 точек от числа прямоугольников

![answering.png](data/img/answering.png)

Сумарное время работы (обработка + ответ)

![total.png](data/img/total.png)
...
При _N <= 300_ алгоритм работает быстрее всех. Однако при дальнейшем возрастании числа прямоугольников имеет смысл 
...
Хотя из графика 2 можно заметить, что константа для ответа у алгоритма на карте меньше чем у дерева, это можно 
объяснить тем, обращение в массив для карты происходит быстрее, чем спуск до элемента по дереву.

Так же суммарное время сильно зависит от числа точек. Так например при M = 1000 "побеждает" наивный алгоритм
а при M = 10000, что отражено в графиках, алгоритм на дереве.